...
describe('SqlDocumentClient (SQLite in-memory)', () => {
    ...
    it('translates put and get parameters correctly', async () => {
        await client.put({
            ...
            Item: { guildId: '123', name: 'test' } // PK depends on TABLE_CONFIGS -> 'guildId' (wait, wait, the client maps PKs using a map)
        }).promise();

        const result = await client.get({
            TableName: TABLE_CONFIGS,
            Key: { guildId: '123' } // Actually it uses Object.keys(params.Key)[0]
        }).promise();

        expect(result.Item).toEqual({ guildId: '123', name: 'test' });
    });
